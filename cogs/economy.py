...
class EconomyCog(commands.Cog):
    ...
    async def topbal(self, ctx: commands.Context):
        ...
        try:
            ...
            sorted_users = sorted(
                self.bank.keys(), key=lambda x: self.get(x), reverse=True
            )
            ...
            for i in range(0, len(top_users), 10):  # 10 users per page
                ...
                for idx, user_id in enumerate(top_users[i : i + 10]):
                    embed.add_field(
                        name=f"{i+idx+1}. {user.name or 'Unknown User' if (user:=self.bot.get_user(int(user_id))) else 'Unknown User'}",
                        value=f"{self.get(user_id):,} Eden coins",
                        inline=False,
                    )
    ...
    async def setbal(self, ctx: commands.Context, member: MemberLike, coins: int):
        ...
        await ctx.send(
            f"{member.mention if isinstance(member, Member) else member} 's balance is {coins} eden coins"
        )
    ...
    async def win(self, ctx: commands.Context):
        ...
        if self.get(ctx.author) < price:
            await ctx.send(
                f"You do not have enough coins to win. You need at least {price:,} eden coins."
            )
            return
        else:
    ...
    async def slots(self, ctx: commands.Context):
        ...
        if self.get(ctx.author) < cost:
            await ctx.send(
                f"You do not have enough coins, you need {cost:,} to participate"
            )
            return
        if bot_choice == 6:
    ...
    async def steal(self, ctx: commands.Context, member: Member):
        ...
        reward = random.randint(
            target_balance // 20, target_balance // 10
        )  # Steal between 5% and 20% of the target's balance
    ...
    async def daily(self, ctx: commands.Context):
        ...
        # Debug print
        print(
            f"[DEBUG] {user.display_name} | Δ: {delta.total_seconds():.2f}s | Streak: {streak}"
        )
    ...
    async def gamble(self, ctx: commands.Context):
        ...
        if random.randint(1, chance) == 3:
            # User wins
            win_amount = min(
                1_000_000, self.jackpot["jackpot"] * 20
            )  # Maximum win amount is 1 million
            self.set(userid, balance + win_amount)
            self.jackpot["jackpot"] = max(
                10_000, self.jackpot["jackpot"] - win_amount
